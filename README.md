...
Текущий выпуск — **[0.2.8](https://github.com/jadieify-hub/krs-acquiring-monitor/releases/tag/v0.2.8)**: сглаженный текст, настройка шрифтов и цветов, более строгая привязка к основному окну Frontol и запрос отчёта без всплывающих сообщений. Ручная сверка запускается двойным щелчком по любой сумме. Ширина меняется за правый край, шрифт, жирность и цвета — в настройках с живым предпросмотром.
...
- появляется поверх активного основного окна Frontol примерно за секунду и скрывается в остальных программах, кроме предпросмотра в настройках;
- скрывается при активности другого окна Frontol; после возврата к основному окну появляется в сохранённом положении;
...
1. Скачайте `KRS-AcquiringMonitor-0.2.8-setup.exe` из [GitHub Releases](https://github.com/jadieify-hub/krs-acquiring-monitor/releases/tag/v0.2.8).
...
1. Скачайте архив `KRS-AcquiringMonitor-0.2.8-win-x86.zip` из [GitHub Releases](https://github.com/jadieify-hub/krs-acquiring-monitor/releases/tag/v0.2.8).
...
## Известные ограничения 0.2.8

- поддерживается только Сбербанк UPOS/pilot_nt.dll;
- основное окно Frontol определяется по заголовку, процессу и признакам окна Windows; служебное окно без владельца с тем же заголовком `Frontol v.` может требовать уточнения распознавания на конкретной кассе;
...
powershell -NoProfile -ExecutionPolicy Bypass -File build/build-release.ps1 -Version 0.2.8
~~~
