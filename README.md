...
Текущая версия — **[0.2.9](https://github.com/jadieify-hub/krs-acquiring-monitor/releases/tag/v0.2.9)**. Исправлена регрессия распознавания Frontol из 0.2.8: главное окно определяется по проверенному классу `TfrmMain`, его владелец больше не мешает распознаванию, а прежняя привязка положения сохранена. Сглаживание, шрифты, жирность, цвета и запрос отчёта без всплывающих сообщений остаются. Ручная сверка запускается двойным щелчком по любой сумме, ширина меняется за правый край.
...
1. Скачайте [KRS-AcquiringMonitor-0.2.9-setup.exe](https://github.com/jadieify-hub/krs-acquiring-monitor/releases/download/v0.2.9/KRS-AcquiringMonitor-0.2.9-setup.exe) из официального GitHub Release. Перед установкой поверх старой версии завершите монитор через меню трея.
...
1. Скачайте [KRS-AcquiringMonitor-0.2.9-win-x86.zip](https://github.com/jadieify-hub/krs-acquiring-monitor/releases/download/v0.2.9/KRS-AcquiringMonitor-0.2.9-win-x86.zip) из официального GitHub Release.
...
Установщик 0.2.9 можно запустить вручную поверх предыдущей установленной версии. Если старый монитор не закрывается штатно, системный Restart Manager принудительно завершает процессы, использующие заменяемые файлы монитора. Это может занять десятки секунд; в 0.2.9 механизм закрытия не менялся. Перед ручной установкой закройте монитор через трей в паузе без банковских операций. Frontol и отдельная portable-копия не входят в список закрываемых процессов. Несохранённые изменения старого окна настроек при принудительном завершении могут потеряться; сохранённые настройки и суммы остаются в профиле пользователя.

Для первого перехода с 0.2.2 и старее рекомендуется запустить установщик 0.2.9 вручную: в старых версиях ещё нет периодической проверки и исправлений завершения процесса. Portable-версию обновляйте заменой файлов после выхода через трей.
...
powershell -NoProfile -ExecutionPolicy Bypass -File build/build-release.ps1 -Version 0.2.9
~~~
